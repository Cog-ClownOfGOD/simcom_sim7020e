...
static struct sim7020e_data mdata;
...
static struct sim7020e_gnss_data gnss_data;
...
{
	struct sim7020e_data *data = dev->data;
	uint32_t hash_value;
...
/**
 * Changes the operating state of the sim7020e.
 *
 * @param state The new state.
 */
static void change_state(enum sim7020e_state state)
{
...
/**
 * Get the current operating state of the sim7020e.
 *
 * @return The current state.
 */
static enum sim7020e_state get_state(void)
{
	return mdata.state;
}
...
static int offload_connect(void *obj, const struct sockaddr *addr, socklen_t addrlen)
{
	...
	if (get_state() != SIM7020E_STATE_NETWORKING) {
		return -EAGAIN;
	}
...
MODEM_CMD_DEFINE(on_urc_ftpget)
{
	...
	/* Transfer finished. */
	if (error == 0) {
		mdata.ftp.state = SIM7020E_FTP_CONNECTION_STATE_FINISHED;
	} else if (error == 1) {
		mdata.ftp.state = SIM7020E_FTP_CONNECTION_STATE_CONNECTED;
	} else {
		mdata.ftp.state = SIM7020E_FTP_CONNECTION_STATE_ERROR;
	}
...
/*
 * Possible responses by the sim7020E.
 */
...
static int modem_autobaud(void)
{
	...
	while (boot_tries++ <= MDM_BOOT_TRIES) {
		...
		/*
		 * The sim7020e has a autobaud function.
...
int mdm_sim7020e_query_gnss(struct sim7020e_gnss_data *data)
{
	int ret;
...
int mdm_sim7020e_start_gnss(void)
{
	int ret;

	change_state(SIM7020E_STATE_INIT);
...
int mdm_sim7020e_ftp_get_read(char *dst, size_t *size)
{
	int ret;
	...
	/* Some error occurred. */
	if (mdata.ftp.state == SIM7020E_FTP_CONNECTION_STATE_ERROR ||
	    mdata.ftp.state == SIM7020E_FTP_CONNECTION_STATE_INITIAL) {
		return SIM7020E_FTP_RC_ERROR;
	}
	...
	if (mdata.ftp.state == SIM7020E_FTP_CONNECTION_STATE_FINISHED) {
		*size = 0;
		return SIM7020E_FTP_RC_FINISHED;
	} else if (mdata.ftp.state == SIM7020E_FTP_CONNECTION_STATE_ERROR) {
		*size = 0;
		return SIM7020E_FTP_RC_ERROR;
	}
	...
	*size = mdata.ftp.nread;
	...
	return SIM7020E_FTP_RC_OK;
}

int mdm_sim7020e_ftp_get_start(const char *server, const char *user, const char *passwd,
			      const char *file, const char *path)
{
	...
	/* Start network. */
	ret = mdm_sim7020e_start_network();
	if (ret < 0) {
	...
	if (mdata.ftp.state != SIM7020E_FTP_CONNECTION_STATE_CONNECTED) {
 ...
 */
static int mdm_decode_pdu(const char *pdu, size_t pdu_len, struct sim7020e_sms *target_buf)
{
	size_t index;
...
int mdm_sim7020e_read_sms(struct sim7020e_sms_buffer *buffer)
{
	int ret;
...
int mdm_sim7020e_delete_sms(uint16_t index)
{
	int ret;
...
static int modem_setup(void)
{
	...
	change_state(SIM7020E_STATE_NETWORKING);

error:
	return ret;
}

int mdm_sim7020e_start_network(void)
{
	change_state(SIM7020E_STATE_INIT);
	return modem_setup();
}

int mdm_sim7020E_power_on(void)
{
	return modem_autobaud();
}

int mdm_sim7020E_power_off(void)
{
	int tries = 5;
	int autobaud_tries;
	int ret = 0;
	...
	ret = modem_cmd_send(&mctx.iface, &mctx.cmd_handler, NULL, 0U, "AT", &mdata.sem_response,
			     K_MSEC(1000));
	if (ret < 0) {
		change_state(SIM7020E_STATE_OFF);
		return 0;
	}
...
const char *mdm_sim7020e_get_manufacturer(void)
{
	return mdata.mdm_manufacturer;
}

const char *mdm_sim7020e_get_model(void)
{
	return mdata.mdm_model;
}

const char *mdm_sim7020e_get_revision(void)
{
	return mdata.mdm_revision;
}

const char *mdm_sim7020e_get_imei(void)
{
...
NET_DEVICE_DT_INST_OFFLOAD_DEFINE(0, modem_init, NULL, &mdata, NULL,
				  CONFIG_MODEM_SIMCOM_SIM7020E_INIT_PRIORITY, &api_funcs,
				  MDM_MAX_DATA_LENGTH);
